export default function SobreElMovimiento() {
  return (
    <div className="mx-auto max-w-3xl px-6 py-24">
      ...
      {/* PLACEHOLDER: título real */}
      <h1 className="mt-4 font-display text-4xl font-semibold text-ink">
        Nuestra historia
      </h1>

      {/* PLACEHOLDER: historia real del movimiento */}
      ...
      <div className="mt-16 grid gap-10 sm:grid-cols-2">
        <div>
          ...
          <p className="mt-3 text-ink/70">
            Texto placeholder de la misión del movimiento.
          </p>
        </div>
        <div>
          <h2 className="font-display text-2xl font-semibold text-ink">
          ...
          {/* PLACEHOLDER */}
          <p className="mt-3 text-ink/70">
            Texto placeholder de la visión del movimiento.
          </p>
        </div>
      </div>
      ...
      <div className="mt-16">
        ...
        {/* PLACEHOLDER: lista real de valores */}
        <ul className="mt-4 grid gap-3 sm:grid-cols-2">
          {["Cercanía", "Transparencia", "Compromiso", "Innovación"].map(
            (valor) => (
              <li
                key={valor}
                className="rounded-lg border border-ink/10 px-4 py-3 text-ink/80"
              >
                {valor}
              </li>
            )
